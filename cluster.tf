...
resource "google_container_cluster" "gke" {
  ...
  location = google_compute_subnetwork.subnet.0.region
...
resource "google_container_cluster" "gke-2" {
  ...
  project  = local.project_id
  name     = "${var.prefix}-${random_id.postfix.hex}-2"
  location = google_compute_subnetwork.subnet.1.region
...
resource "google_container_node_pool" "gateway-np-2" {
  project = local.project_id
  name    = "${var.prefix}-np-gateway-2"
  cluster = google_container_cluster.gke-2.self_link
...
// Workload nodepool
resource "google_container_node_pool" "np-int-2" {
  project = local.project_id
  name    = "${var.prefix}-workload-2"
  cluster = google_container_cluster.gke-2.self_link
